...
function [f] = p53Cinema_singleCellAnnotation_gui_fileManager(master)
%% Create the figure
%
...
htextPimaryChannel = uicontrol('Style','text','Units','characters',...
    'FontSize',10,'FontName','Arial','HorizontalAlignment','right',...
    'String','Channel','Position',[hx, hy, hwidth, hheight],...
    'parent',f);
hpopupPrimaryChannel = uicontrol('Style','popupmenu','Units','characters',...
    'FontSize',10,'FontName','Arial','HorizontalAlignment','right',...
    'String','Select channel','Position',[hx + hmargin + hwidth, hy, hwidth, hheight],...
    'Enable', 'off', 'Callback',{@popupChannel_Callback}, 'parent',f);
...
hy = hy - hheight - hmargin_short;
htextCellId = uicontrol('Style','text','Units','characters',...
    'FontSize',10,'FontName','Arial','HorizontalAlignment','right',...
    'String','Cell id','Position',[hx, hy, hwidth, hheight],...
    'parent',f);
hpopupCellId = uicontrol('Style','popupmenu','Units','characters',...
    'FontSize',10,'FontName','Arial','HorizontalAlignment','right',...
    'String','Select cell id','Position',[hx + hmargin + hwidth, hy, hwidth, hheight],...
    'Enable', 'off', 'parent',f);
...
hpushbuttonLoadData = uicontrol('Style','pushbutton','Units','characters',...
    'FontSize',10,'FontName','Arial','HorizontalAlignment','right',...
    'String','Load','Enable','off','Position',[hx + hmargin + hwidth, hy, hwidth * 0.75, hheight],...
...
handles.hpopupPrimaryChannel = hpopupPrimaryChannel;
...
%
    ...
    function popupChannel_Callback(~,~)
        populateCell;
    end
    ...
    function pushbuttonLoadData_Callback(~,~)
        ...
        master.obj_fileManager.setSelectedChannel(getCurrentPopupString(hpopupPrimaryChannel));
        master.obj_fileManager.setSelectedCell(str2double(getCurrentPopupString(hpopupCellId)));
        ...
        master.obj_fileManager.setPredictionMode(getCurrentPopupString(hpopupPredictionMode));
...
%
    function populatePositionChannelCell
    ...
    function populateChannelCell
        ...
        previousValue = getCurrentPopupString(hpopupPrimaryChannel);
        set(hpopupPrimaryChannel, 'Value', 1);
        set(hpopupPrimaryChannel, 'String', availableChannels);
        set(hpopupPrimaryChannel, 'Value', getPopupIndex(hpopupPrimaryChannel, previousValue));
        set(hpopupPrimaryChannel, 'Enable', 'on');
    ...
    function populateCell
        database = master.obj_fileManager.database;
        availableCells = unique( ...
            database.cell_id(strcmp(database.group_label, getCurrentPopupString(hpopupGroupLabel)) & ...
            database.position_number == str2double(getCurrentPopupString(hpopupStagePosition)) & ...
            strcmp(database.channel_name, getCurrentPopupString(hpopupPrimaryChannel))));
        previousValue = getCurrentPopupString(hpopupCellId);
        set(hpopupCellId, 'Value', 1);
        set(hpopupCellId, 'String', availableCells);
        set(hpopupCellId, 'Value', getPopupIndex(hpopupCellId, previousValue));
        set(hpopupCellId, 'Enable', 'on');
    end
