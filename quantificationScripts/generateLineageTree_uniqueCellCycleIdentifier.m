function lineageTree = generateLineageTree_uniqueCellCycleIdentifier(centroidsTracks, centroidsDivisions)
...
for i=1:length(trackedCells)
    ...
    currentDivisions = centroidsDivisions.getCellTrack(trackedCells(i));
    divisionTiming = find(currentDivisions(:,1) > 0);
    for j=1:length(divisionTiming)
        if(divisionTiming(j) < max(trackedPositions))
            lineageTree(i, (divisionTiming(j)+1):max(trackedPositions)) = lineageTree(i, (divisionTiming(j)+1):max(trackedPositions)) + 1/10;
        end
    end
end
