...
%    continue; 
    ...
    siz = 111;
    for j=1:2:size(currentCentroids,1)
        currentCell = validCells(j);
        
        segmentationImage = IntensityImages_blurred{segmentationChannelIndex};
        
        [zoomImage, newCentroid1, newCentroid2]=GetBlock(segmentationImage,currentCentroids(j,:),currentCentroids(j+1,:),siz);
        zoomImage=(imfilter(zoomImage,fspecial('gaussian',5,2),'replicate'));
        binaryMask=autoscale(zoomImage);
        
        g3_1 = GetBlock(binaryMask,newCentroid1,newCentroid1,11);
        g3_2 = GetBlock(binaryMask,newCentroid2,newCentroid2,11);
        
        g3 = horzcat(g3_1(:), g3_2(:));
        ...
        queryCentroids = vertcat(newCentroid1, newCentroid2);
        distances = pdist2(local_centroids(1:2,:), queryCentroids);
        configuration = sum(distances,2);
        selected = find(configuration == max(configuration), 1, 'first');
        if(selected == 1)
            singleCellTracks_foci{1}(validCells(j),i) = local_values(1);
            singleCellTracks_focus_row(validCells(j),i) = currentCentroids(j,1) + newCentroid1(1) - local_centroids(1,1);
            singleCellTracks_focus_col(validCells(j),i) = currentCentroids(j,2) + newCentroid1(2) - local_centroids(1,2);
            
            singleCellTracks_foci{1}(validCells(j+1),i) = local_values(2);
            singleCellTracks_focus_row(validCells(j+1),i) = currentCentroids(j+1,1) + newCentroid2(1) - local_centroids(2,1);
            singleCellTracks_focus_col(validCells(j+1),i) = currentCentroids(j+1,2) + newCentroid2(2) - local_centroids(2,2);
        else
            singleCellTracks_foci{1}(validCells(j),i) = local_values(2);
            singleCellTracks_focus_row(validCells(j),i) = currentCentroids(j,1) + newCentroid1(1) - local_centroids(2,1);
            singleCellTracks_focus_col(validCells(j),i) = currentCentroids(j,2) + newCentroid1(2) - local_centroids(2,2);
            
            singleCellTracks_foci{1}(validCells(j+1),i) = local_values(1);            
            singleCellTracks_focus_row(validCells(j+1),i) = currentCentroids(j+1,1) + newCentroid2(1) - local_centroids(1,1);
            singleCellTracks_focus_col(validCells(j+1),i) = currentCentroids(j+1,2) + newCentroid2(2) - local_centroids(1,2);
        end
        
%         localDistance_1 = pdist2(local_centroids, newCentroid1); local_possible1 = find(localDistance_1 <= interCentroidDistance);
%         localDistance_2 = pdist2(local_centroids, newCentroid2); local_possible2 = find(localDistance_2 <= interCentroidDistance);        
...
%             subImage=GetBlock(IntensityImages_blurred{w},currentCentroids(j,:),currentCentroids(j+1,:),siz);
