function measurements = getDatasetTraces_binucleatedCells(database, rawdata_path, tracking_path, ffpath, measurementChannels, segmentationChannel)
    ...
    singleCellTracks_focus_row = -ones(maxCells, numTimepoints);
    singleCellTracks_focus_col = -ones(maxCells, numTimepoints);
    ...
    for i=1:length(trackingFiles)
        ...
        [traces_mean, traces_median, traces_foci, traces_area, traces_solidity, traces_row, traces_col] = getSingleCellTracks_binucleatedCells(database, rawdata_path, selectedGroup, selectedPosition, measurementChannels, segmentationChannel, centroidsTracks, ff_offset, ff_gain);
        ...
        traces_solidity = fillLineageInformation(traces_solidity, centroidsTracks, centroidsDivisions);
        traces_row = fillLineageInformation(traces_row, centroidsTracks, centroidsDivisions);
        traces_col = fillLineageInformation(traces_col, centroidsTracks, centroidsDivisions);
        ...
        singleCellTracks_solidity(subsetIndex,:) = traces_solidity;
        singleCellTracks_focus_row(subsetIndex,:) = traces_row;
        singleCellTracks_focus_col(subsetIndex,:) = traces_col;
        ...
        tempMatrix = singleCellTracks_foci{1}(1:(counter-1),:);
        save('temp_measurements.mat', 'tempMatrix');
    end
    for w=1:length(measurementChannels)
    ...
    measurements.singleCellTracks_focus_row = singleCellTracks_focus_row(1:(counter-1),:);
    measurements.singleCellTracks_focus_col = singleCellTracks_focus_col(1:(counter-1),:);
