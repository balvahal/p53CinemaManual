function cellCycles = getSingleCellCycleMatrix_includeIncomplete_leftAlign(traces, divisions, numCycles)
    ...
    for i=1:size(traces,1)
        ...
        if(length(divisionEvents) >= numCycles)
            for j=1:(length(divisionEvents)-numCycles+1)
                if(j + numCycles <= length(divisionEvents)) 
                    ending = divisionEvents(j+numCycles);
                else
                    ending = size(traces,2);
                end
                
                subTrace = traces(i,divisionEvents(j):ending);
