...
for i=1:length(uniqueGroups)
    ...
    parfor s=1:length(uniquePositions)
        if(exist(fullfile(outputPath, sprintf('%s_s%d.txt', currentGroup, uniquePositions(s))), 'file'))
            continue;
        end
        ...
        writetable(measurements, fullfile(outputPath, sprintf('%s_s%d.txt', currentGroup, uniquePositions(s))), 'Delimiter', '\t');
    end
end
end