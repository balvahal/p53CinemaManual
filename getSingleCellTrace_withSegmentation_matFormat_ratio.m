function singleCellTracks = getSingleCellTrace_withSegmentation_matFormat_ratio(rawdatapath, segmentationpath, database, group, position, measurementChannels, segmentationChannel, centroids, measurementParameter)
...
for i=1:numTimepoints
    ...
    if(~isempty(measurementChannels))
        measurementFile1 = getDatabaseFile2(database, group, measurementChannels{1}, position, i);
        measurementFile2 = getDatabaseFile2(database, group, measurementChannels{2}, position, i);
    else
        measurementFile = [];
    end
